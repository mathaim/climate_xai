"""Full-record matry_L8 region activations (mean+max pooled per concept) so we can
characterize parent/child concepts by region, AR intensity, and time."""
...
def main():
    ...
    setup=region_node_setup(); m,c,fmin,frng=load_sae("matry_L8",dev)   # matry: fmin/frng not None
    ...
    for n,i in enumerate(range(1,TOTAL+1)):
        ...
        for r in REGIONS:
            ...
            IVT[r][n]=max_map[r].get(int(i),np.nan); IVTM[r][n]=mean_map[r].get(int(i),np.nan)
        if n%2000==0: print(n,"/",TOTAL,"miss",miss,flush=True)
    for r in REGIONS:
        np.savez(f"{OUT}/track_matry_{r}.npz",A_max=AMAX[r],A_mean=AMEAN[r],ivt=IVT[r],ivt_mean=IVTM[r],tindex=tindex,month=month)
