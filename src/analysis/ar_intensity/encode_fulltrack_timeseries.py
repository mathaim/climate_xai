"""Per-region (T,4096) Plain-L8 region-mean activations over the full training record.
IVT (max+mean) is read from ar_intensity_full.parquet; only activations are encoded here."""
...
def main():
    ...
    A = {r: np.zeros((TOTAL, 4096), np.float32) for r in REGIONS}
    ...
    for r in REGIONS:
        np.savez(f"{OUT}/track_{r}.npz", A=A[r], ivt=IVT[r], ivt_mean=IVTM[r],
                 tindex=tindex, month=month)
