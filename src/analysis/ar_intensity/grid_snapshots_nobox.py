...
for i in range(n):
    ...
    A.gridlines(lw=.2, color="#ccc"); A.set_title(labels[i], fontsize=13, fontweight="bold")
    B = geo(1, i)
    ...
    B.gridlines(lw=.2, color="#ccc"); axI.append(A); axA.append(B)
    if i == 0:
...
fig.savefig(out.replace(".png","_nobox.png"), dpi=150, bbox_inches="tight"); print("saved", out)
