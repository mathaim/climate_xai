"""Mean-footprint figures: concept mean activation (global+zoom) + AR-occurrence frequency."""
...
def main():
    ...
    for r in REGIONS:
        cc=int(z[f"{r}_concept"][0]); val=z[f"{r}_act"]; freq=z[f"{r}_freq"]; n=int(z[f"{r}_n"][0])
        ...
        A=geo(fig,3); A.set_extent(ext,crs=PC); A.set_title("AR occurrence frequency — zoom")
        fp=A.contourf(MLON,MLAT,freq,levels=np.linspace(0,max(freq.max(),0.01),11),cmap="YlGnBu",transform=PC,zorder=2)
        A.gridlines(draw_labels=True,lw=.3,color="#bbb"); fig.colorbar(fp,ax=A,shrink=.65,label="fraction of ARs")
        fig.suptitle(f"{r}: AR-specific concept {cc} mean footprint over {n} ARs",y=1.03)
