"""Region AR-concept activation vs AR presence: scatter map + spatial-match correlation."""
import numpy as np
...
def main():
    z=np.load(f"{OUT}/mean_footprint_{SAE}.npz"); d=np.load(NPZ); setup=region_node_setup(); grid_lon=np.arange(0,360,0.25); scores=[]
    for r in REGIONS:
        cc=int(z[f"{r}_concept"][0]); val=z[f"{r}_act"]; nlat=z[f"{r}_nlat"]; nlon=np.array([conv(x) for x in z[f"{r}_nlon"]]); freq0=z[f"{r}_freq"]; n=int(z[f"{r}_n"][0])
        s=setup[r]; arn=freq0[s["li"],s["ji"]]
        rm=float(np.corrcoef(val,arn)[0,1]) if val.std()>0 and arn.std()>0 else float("nan"); scores.append((r,cc,rm))
        cfg=REGIONS[r]; la=cfg["lat"]; lons=cfg["lon"]; xs=[conv(x) for seg in lons for x in seg]; ext=[min(xs)-3,max(xs)+3,la[0]-3,la[1]+3]
        ...
        sc=A.scatter(nlon,nlat,c=val,s=70,cmap="magma",vmin=0,vmax=max(np.percentile(val,99),1e-6),transform=PC,zorder=3,edgecolor="none")
        A.gridlines(draw_labels=True,lw=.3,color="#bbb"); fig.colorbar(sc,ax=A,shrink=.6,label="mean activation")
