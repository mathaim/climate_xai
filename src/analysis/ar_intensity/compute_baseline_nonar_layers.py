"""Non-AR region firing rates for plain_L0 and plain_L15 (region-node reads, sampled)."""
...
SAES_TO_DO=os.environ.get("NONAR_SAES","plain_L0,plain_L15").split(","); N_SAMPLE=10000; DEFS=["binary","magnitude","top10"]
def main():
    ...
    for SAE in SAES_TO_DO:
        ...
        for i,ti in enumerate(keep):
            ...
            with torch.no_grad():
                acts=encode(m,c["arch"],torch.from_numpy(xr).to(dev)); ge0=(acts>0).float(); cl=acts.clamp_min(0); gt=(acts>p90).float()
                for r in REGIONS:
                    if ti in noar[r]:
                        idx=rt[r]
