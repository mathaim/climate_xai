"""Climate-style AR view: IVT magnitude + transport vectors + 250 contour, beside the
intensity concept. Events picked from the FULL 40-yr record (track_pool), spread across time."""
...
def make_map(r,cc,dt,m,c,dev,levels,qi,ui,vi,lat_i,lon_i,dp):
    ...
    fig.savefig(f"{PLOTS}/climate_{r}_c{cc}_{dt:%Y%m%d}.png",dpi=160,bbox_inches="tight"); plt.close(fig)
...
def main():
    ...
    for r in REGIONS:
        cc=CONCEPT[r]
        d=np.load(f"{TRACK}/track_pool_{r}.npz"); act=d["A_max"][:,cc].astype(float); tindex=d["tindex"]; del d
        for ch in np.array_split(np.arange(len(act)),NCHUNK):
            ti=int(tindex[ch[np.nanargmax(act[ch])]]); dt=index_to_datetime(ti)
            make_map(r,cc,dt,m,c,dev,levels,qi,ui,vi,lat_i,lon_i,dp)
