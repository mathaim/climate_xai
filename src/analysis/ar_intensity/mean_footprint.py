"""Region-node concept footprint (efficient: reads only region nodes): mean activation + AR freq."""
...
def main():
    ...
    setup=region_node_setup(); era0=np.load(sorted(glob.glob(f"{ERA5_DIR}/era5_inputs_*.npy"))[0]); nlat=era0[:,lat_i]; nlon=era0[:,lon_i]
    ...
    acc={r:np.zeros(len(setup[r]["nodes"])) for r in REGIONS}; freq={r:np.zeros(masks[r].shape[1:]) for r in REGIONS}; cnt={r:0 for r in REGIONS}
    for i,ti in enumerate(keep):
        ...
        for r in rs: acc[r]+=acts[rmap[r],CONCEPT[r]]; freq[r]+=masks[r][int(ti)-1]; cnt[r]+=1
        if (i+1)%500==0: print(f"{i+1}/{len(keep)}",flush=True)
    save={}
    for r in REGIONS:
        nodes=setup[r]["nodes"]
        save[f"{r}_act"]=(acc[r]/max(cnt[r],1)).astype(np.float32)
        save[f"{r}_nlat"]=nlat[nodes].astype(np.float32); save[f"{r}_nlon"]=nlon[nodes].astype(np.float32)
        save[f"{r}_freq"]=(freq[r]/max(cnt[r],1)).astype(np.float32); save[f"{r}_n"]=np.array([cnt[r]]); save[f"{r}_concept"]=np.array([CONCEPT[r]])
    np.savez(f"{OUT}/mean_footprint_{SAE}.npz", **save); print("FOOTPRINT DONE",{r:cnt[r] for r in REGIONS},flush=True)
