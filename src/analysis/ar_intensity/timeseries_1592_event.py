"""W. N. America concept 1592 activation and region IVT over Nov-Dec 2012, with the
2012-11-28 06z AR (the spatial-map snapshot) marked."""
...
WIN = (pd.Timestamp("2012-11-01"), pd.Timestamp("2012-12-31"))
...
def main():
    ...
    l1, = ax.plot(d, iv, color="#2b6cb0", lw=1.3, label="Region max IVT")
    ax.axhline(250, color="#2b6cb0", ls=":", lw=.9)
    ax.set_ylabel("IVT (kg m$^{-1}$ s$^{-1}$)", color="#2b6cb0"); ax.tick_params(axis="y", colors="#2b6cb0")
    ax.set_xlabel("Date  (Nov-Dec 2012)")
    ax2 = ax.twinx()
    l2, = ax2.plot(d, aa, color="#c0392b", lw=1.3, label="Concept 1592 activation")
    ax2.set_ylabel("Concept 1592 activation", color="#c0392b"); ax2.tick_params(axis="y", colors="#c0392b")
    amax = float(aa.max()); ax2.set_ylim(0, amax*1.28)
    ax.axvline(EVENT, color="k", ls="--", lw=1.3)
    ax2.annotate("2012-11-28 06z snapshot", xy=(EVENT, amax*1.0), xytext=(EVENT, amax*1.20),
