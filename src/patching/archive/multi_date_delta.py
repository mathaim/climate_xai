"""Stage 2: removal field for concept 99 at every captured date (validated formula). Resumable."""
...
def main():
    ...
    for f in sorted(glob.glob(f"{PATCH}/x8_*.npy")):
        ...
        if os.path.exists(f"{PATCH}/d99_{tag}.npy"):
            continue
        ...
        with torch.no_grad():
            ...
            zc = code[:, 99].numpy()
            d = (-zc[:, None] * m.W_dec[99].numpy()[None, :]) * (frng[None, :]/(2.0*s))
            np.save(f"{PATCH}/d99_{tag}.npy", d.astype(np.float32))
            print(f"{tag}: fires {(zc>0).sum()}", flush=True)
