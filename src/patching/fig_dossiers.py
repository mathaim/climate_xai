...
for lab, v, col in [("global mean", [152.6,153.9,154.6,155.2], "#34495e"), ("Chile AR peak", [898.9,912.7,931.6,945.2], "#c0392b")]:
    a = ax[3].twinx() if lab == "Chile AR peak" else ax[3]
    a.plot(G, v, "-s", ms=4, color=col, label=lab); a.set_ylabel(f"{lab} IVT", fontsize=8, color=col)
ax[3].set_title("(d) forecast response", fontsize=9, loc="left"); ax[3].grid(alpha=.25)
ax[3].set_xticks(G); ax[3].set_xticklabels(["clamp\ng=0","baseline\ng=1","amplify\ng=2","amplify\ng=3"], fontsize=7)
...
ax[2].plot([1,2,3], [235.3,234.1,232.4], "-s", ms=4, color="#34495e")
ax[2].axvline(1, color="0.9", lw=0.8, ls=":")
ax[2].set_title("(c) Chile box mean IVT: amplifying the parent dries the coast", fontsize=9, loc="left")
ax[2].set_ylabel("IVT (kg/m/s)", fontsize=8); ax[2].grid(alpha=.25)
ax[2].set_xticks(G); ax[2].set_xticklabels(["clamp\ng=0","baseline\ng=1","amplify\ng=2","amplify\ng=3"], fontsize=7)
