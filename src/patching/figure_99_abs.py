"""Two-row concept-99 GLOBAL figure, ALL absolute IVT on one shared scale: top (a) truth,
(b) baseline; bottom (c-e) forecast under remove/clamp/amplify. One colorbar. Env: PNG, DATE."""
import numpy as np, os, matplotlib; matplotlib.use("Agg"); import matplotlib.pyplot as plt; from matplotlib.colors import PowerNorm
PATCH = "/scratch/euh7ys/climate_xai/patching"; OUT = "/scratch/euh7ys/climate_xai/plots"
base = np.load(f"{PATCH}/ivtmap_ho_base.npy"); nlat, nlon = base.shape
lat = np.linspace(-90, 90, nlat); lon = ((np.arange(nlon)*(360.0/nlon) + 180) % 360) - 180
o = np.argsort(lon); lon = lon[o]; prep = lambda a: a[:, o]
Tr = prep(np.load(f"{PATCH}/ivtmap_ho_truth.npy")); B = prep(base)
PAN = [prep(np.load(f"{PATCH}/ivtmap_ho_99_clamp.npy")),
       prep(np.load(f"{PATCH}/ivtmap_ho_99_beta05.npy")),
       prep(np.load(f"{PATCH}/ivtmap_ho_99_amp3.npy"))]
dlabels = ["(c) Removed ($\\beta = 0$)", "(d) Clamped ($\\beta = 0.5$)", "(e) Amplified ($\\beta = 3$)"]
DATE = os.environ.get("DATE", "")
VMIN = float(os.environ.get("VMIN", 0)); GAMMA = float(os.environ.get("GAMMA", 1))
vabs = float(np.nanpercentile(np.concatenate([Tr.ravel(), B.ravel(), *[p.ravel() for p in PAN]]), 99.5))
try:
    import cartopy.crs as ccrs
    proj = {"projection": ccrs.PlateCarree()}; tk = {"transform": ccrs.PlateCarree()}; HAVE = True
except Exception:
    proj = {}; tk = {}; HAVE = False
TITLE, CBLBL, CBTK, DTF = 18, 15, 12, 21
fig = plt.figure(figsize=(15, 5.6))
gs = fig.add_gridspec(2, 6, left=0.05, right=0.89, bottom=0.04, top=0.95, wspace=0.06, hspace=0.12)
tops = [fig.add_subplot(gs[0, 1:3], **proj), fig.add_subplot(gs[0, 3:5], **proj)]
bots = [fig.add_subplot(gs[1, 2*i:2*i+2], **proj) for i in range(3)]
def draw(ax, Z, title):
    im = ax.pcolormesh(lon, lat, Z, cmap="YlGnBu", norm=PowerNorm(GAMMA, vmin=VMIN, vmax=vabs), shading="auto", **tk)
    if HAVE: ax.coastlines(lw=0.5); ax.set_global()
    ax.set_title(title, fontsize=TITLE, loc="left", fontweight="bold"); return im
im = draw(tops[0], Tr, "(a) Ground Truth")
draw(tops[1], B, "(b) Baseline Forecast")
for i, (Z, t) in enumerate(zip(PAN, dlabels)):
    im = draw(bots[i], Z, t)
fig.canvas.draw()
pA = tops[0].get_position(); pT = tops[1].get_position(); pB = bots[2].get_position()
if DATE:
    fig.text((0.05 + pA.x0)/2.0, (pA.y0 + pA.y1)/2.0, DATE.replace("\\n","\n"), fontsize=DTF, ha="center", va="center", fontweight="bold")
cax = fig.add_axes([pB.x1 + 0.012, pB.y0, 0.012, pT.y1 - pB.y0])
cb = fig.colorbar(im, cax=cax); cb.set_label("IVT (kg m$^{-1}$ s$^{-1}$)", fontsize=CBLBL); cax.tick_params(labelsize=CBTK)
out = f"{OUT}/{os.environ.get('PNG', 'tworow_99_abs.png')}"
fig.savefig(out, dpi=170, bbox_inches="tight"); print("saved", out)
